...
> **Состояние на 14 августа 2026.** Поток A закрыт, кроме трёх продуктовых
> пунктов.
>
> | Пункт | Состояние |
> |---|---|
> | A0 baseline | закрыт: Ruff и pre-commit в потоке B, полный прогон — 473 теста зелёные |
> | A1 Перегенерация Плана | закрыт этапом 1 (`1348d77`) |
> | A2 lease heartbeat и fencing | закрыт этапом 1 |
> | A3 идемпотентное application | закрыт этапом 1; остаток про каноничное сообщение — #73 |
> | A4 lifecycle ошибок Статьи | закрыт этапом 1 |
> | A5 конкурентные операции | закрыт этапом 1 |
> | A6 prompt pipeline | **осталось**, нужен грилинг |
> | A7 контракт Источников | **осталось**, нужен грилинг — P0 аудита |
> | A8 Wordstat ranking и rubric | **осталось**, нужен грилинг |
> | A9 provenance и cost | закрыт #74; лимиты бюджета вынесены в поток C |
> | A10 миграции, scheduler, runbook | миграции #71, reconciliation #72; VPS runbook остаётся в #8 |
>
> Подробности этапа 1 — `docs/reviews/2026-08-12-audit-hardening-stage-1.md`.
>
> Три оставшихся пункта продуктовые: в них надо решить, что считается
> подтверждением факта, чем Дзен проверяемо отличается от VC и что ranking
> должен вознаграждать. Заводить их тикетом без грилинга нельзя — получится
> тикет без критерия приёмки.
...
> **Персона исключена из этого пункта — см. ADR-0010.** Кастомная Персона
> сознательно разворачивается в system-промпт без защиты от инъекций: Владелец
> по определению роли имеет доступ к промптам генерации, и защищаться
> system-промптом от того, кто может его править, — театр. Корпус атак не должен
> использовать Персону как вектор, иначе acceptance `0%` недостижим по
> построению. Для Темы, комментария Контент-менеджера и прочего недоверенного
> ввода разделение «system — правила, `INPUT_DATA` — данные» остаётся в силе.
> «Голос» в пунктах выше читать как «Персона» (ADR-0009).
