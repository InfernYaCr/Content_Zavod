...
from content_zavod.personas import (
    DEFAULT_PERSONA_KEY,
    PERSONAS,
...
def test_default_resolves_to_full_practical_marketer_persona() -> None:
    persona, custom_voice = resolve_persona(None)
...
def test_known_preset_roundtrips_through_setting_value() -> None:
    persona, custom_voice = resolve_persona(persona_setting_value("evidence_analyst"))
...
def test_legacy_voice_remains_supported_as_custom_data() -> None:
    persona, custom_voice = resolve_persona("технооптимист-фаундер")

    assert persona is None
    assert custom_voice == "технооптимист-фаундер"
